...
// Worker build: filter namespaces by WORKER_ROUTES whitelist
if (isWorkerBuild) {
    const whitelist = (process.env.WORKER_ROUTES || '').split(',').map(s => s.trim()).filter(Boolean);
    if (whitelist.length) {
        for (const ns of Object.keys(namespacesToProcess)) {
            if (!whitelist.includes(ns)) {
                delete namespacesToProcess[ns];
            }
        }
    }
}
